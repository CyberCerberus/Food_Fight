...
public class Monster extends Character{
    ...
    public Action attack(Character[] enemies) {
        ...
        if(decision > 0){
        	Skill s = skillChoice(skill1, skill2, skill3, random);
        	return new Action(user, this + " used " + s, s.getRaw(), 0.0, 0.0, 0.0, 0.0, 0, false, enemies[targetNum]);
        }
        else{
    ...
    public Skill skillChoice(Skill s1, Skill s2, Skill s3, Random r){
    	int rand;
    	if(skillLength() == 1){
    		return s1;
    	}
    	else if(skillLength() == 2){
    		if(r.nextInt(1) == 0)
    			return s1;
    		else
    			return s2;
